...
HRESULT InitialiseGraphics()
{
	...
	D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, "assets/moon.jpg", NULL, NULL, &g_pTexture0, NULL);
	//D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, "assets/lucy.jpg", NULL, NULL, &g_pTexture0, NULL);
...
void RenderFrame(void)
{
	...
	Model_2->SetXPos(0.7);
	Model_2->SetYPos(0.001);
	Model_2->SetZPos(0.001);

	Model_2->draw(&view, &projection);

	Model_1->LookAt_ZX(Model_2->GetXPos(), Model_2->GetZPos());

	if (Model_1->CheckCollision(Model_2) == false)
		Model_1->move_forward(0.001);
	if (Model_1->CheckCollision(Model_2) == true)
		Model_1->move_forward(0);
