...
{
	XMMATRIX WorldViewProjection;   // 64 bytes (4*4 = 16, 16 * 4 = 64)
	float scale;					// 4 bytes	
...
void RenderFrame(void)
{
	...
	CONSTANT_BUFFER0 cb0_values;
	cb0_values.RedAmount = 0.9f;//50% of vertex red value
	//int i = rand() % 5; // changing the value of the scale every frame to a random number between 1-5
	cb0_values.scale = 0.5f; // assign random number to the scale (changed to hard code atm to make sure the other things are orking as intended)
